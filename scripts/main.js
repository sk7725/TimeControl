...
function addSpeed(t, speed) {
  ...
  b.label(
    prov(
      () => (current == speed ? "[#a9d8ff]" : "[white]") + "x" + speed + "[]"
    )
  );
  b.clicked(() => {
...
function addSpeedAlt(t, speed, speed2) {
  ...
  b.clicked(() => {
    if (current == speed) {
      Time.setDeltaProvider(() =>
        Math.min(Core.graphics.getDeltaTime() * 60 * speed2, 3 * speed2)
      );
      current = speed2;
...
function addSpeedThree(t, speed, speed2, speed3) {
    ...
    prov(
      () =>
        (current == speed
          ...
          : "[white]") +
        "x" +
        (current == speed2 ? speed2 : current == speed3 ? speed3 : speed) +
        "[]"
    )
  );
  b.clicked(() => {
  ...
  b.update(() => {
    if (b.isPressed()) {
      ...
      if (h3 > longPress) {
        Time.setDeltaProvider(() =>
          Math.min(Core.graphics.getDeltaTime() * 60 * speed3, 3 * speed3)
        );
        current = speed3;
...
function addMini(t, speedlist, showlist) {
  ...
  b.label(
    prov(
      () =>
        (current == 1 ? "[#a9d8ff]" : current > 1.9 ? "[accent]" : "[orange]") +
        showlist[mode] +
        "[]"
    )
  );
  b.clicked(() => {
    ...
    Time.setDeltaProvider(() =>
      Math.min(
        Core.graphics.getDeltaTime() * 60 * speedlist[mode],
        3 * speedlist[mode]
      )
    );
  ...
  b.update(() => {
    ...
    b.setColor(
      current == 1 ? Pal.lancerLaser : current > 1.9 ? Pal.accent : Color.orange
    );
  });
  return t
    .add(b)
...
function addTable(table) {
  ...
  table.visibility = () =>
    !folded && Vars.ui.hudfrag.shown && !Vars.ui.minimapfrag.shown();
}
...
function addMiniT(table) {
  table.table(
    ...
    cons((t) => {
      t.background(Tex.buttonEdge3);
      if (Vars.mobile)
        addMini(t, speedArr, [".25", ".5", "x1", "x2", "x4", "×?"]);
      else
        addMini(t, speedArr, ["x.25", "x0.5", "x1", "x2", "x4", "x8+"]).width(
          60
        );
    })
  );
